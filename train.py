...
def trainFAN(args):
 ...
 \
    model1 = FCDenseNet103(n_classes=68)
    
    modelfilename = 'checkpoint/Test_image/model_best.pth.tar'

    face_alignment_net = torch.nn.DataParallel(model1).cuda()
...
def train(loader, model, criterion, optimizer, netType, debug=False, flip=True):
    ...
    return losses.avg, acces.avg, model


def validate(loader, model, criterion, netType, debug, flip= True):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    acces = AverageMeter()
