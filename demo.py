    ...
    if datapath.endswith('300W_LP'):
        ...
        for d in dirs:
            files = [
                ...
                if f.endswith('t7') and f.find('test') != -1
            ]
            lines.extend(files)
            #print(files)
        ...
        for i, f in enumerate(lines):
            if pointType == '2D':
                pts = torchfile.load(osp.join(base_dir, f.split('_')[0], f[:-3] + '.t7'))[0]
                #print(type(pts))
            else:
                pts = torchfile.laod(osp.join(base_dir, f.split('_')[0], f[:-4] + '.t7'))[1]
...
if __name__ == "__main__":
    ...
    preds = torch.from_numpy(loadpreds_if_exists(osp.join(save_dir, 'preds_best.mat')))
    ...
    if dataset == 'LS3D-W' or dataset == '300VW-3D':
        ...
        print("FINAL: Mean Error: {}. AUC: {}".format(torch.mean(dists) * 100., 2), auc)
